...
class TestCLI:
    ...
    def test_version_command(self) -> None:
        ...
        assert "MIDI Markdown" in result.stdout
    ...
    def test_compile_with_format_0_option(self, tmp_path: Path, valid_fixtures_dir: Path) -> None:
        """Test compile command with --format 0 (single-track)."""
        import mido
        ...
        result = runner.invoke(
            app,
            ["compile", str(input_file), "-o", str(output_file), "--format", "0"],
        )
    ...
    def test_compile_with_format_1_option(self, tmp_path: Path, valid_fixtures_dir: Path) -> None:
        """Test compile command with --format 1 (multi-track) explicit."""
        import mido
        ...
        result = runner.invoke(
            app,
            ["compile", str(input_file), "-o", str(output_file), "--format", "1"],
        )
    ...
    def test_compile_with_format_2_option(self, tmp_path: Path, valid_fixtures_dir: Path) -> None:
        """Test compile command with --format 2 (independent sequences)."""
        import mido
        ...
        result = runner.invoke(
            app,
            ["compile", str(input_file), "-o", str(output_file), "--format", "2"],
        )
    ...
    def test_compile_format_default_is_1(self, tmp_path: Path, valid_fixtures_dir: Path) -> None:
        """Test compile command without --format defaults to Format 1."""
        import mido
