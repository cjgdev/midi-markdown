...
class Validator:
    ...
    def validate_midi_value(value: int, min_val: int = 0, max_val: int = 127) -> None:
        ...
        if not (min_val <= value <= max_val):
            raise ValidationError(
                f"MIDI value {value} out of range [{min_val}-{max_val}]",
                error_code="E200",
    ...
    def validate_note(note: int | str) -> int:
        ...
        raise ValidationError(
            f"Note must be string or integer, got {type(note).__name__}",
            error_code="E201",
    ...
    def validate_velocity(velocity: int) -> None:
        ...
        if not (MIDI_MIN <= velocity <= MIDI_MAX):
            raise ValidationError(
                f"Velocity {velocity} out of range [{MIDI_MIN}-{MIDI_MAX}]",
                error_code="E206",
    ...
    def validate_cc_value(value: int | dict) -> None:
        ...
        if isinstance(value, dict):
            if value.get("type") in ("ramp", "random"):
                ...
                return
            raise ValidationError(
                f"Unknown CC expression type: {value.get('type')}",
                error_code="E203",
        ...
        if not (MIDI_MIN <= value <= MIDI_MAX):
            raise ValidationError(
                f"CC value {value} out of range [{MIDI_MIN}-{MIDI_MAX}]",
                error_code="E208",
    ...
    def validate_program(program: int) -> None:
        ...
        if not (MIDI_MIN <= program <= MIDI_MAX):
            raise ValidationError(
                f"Program {program} out of range [{MIDI_MIN}-{MIDI_MAX}]",
                error_code="E209",
    ...
    def validate_tempo(bpm: int | float) -> None:
        ...
        if not isinstance(bpm, (int, float)):
            raise ValidationError(
                f"Tempo must be a number, got {type(bpm).__name__}",
                error_code="E201",
        ...
        if not (TEMPO_MIN <= bpm <= TEMPO_MAX):
            ...
            raise ValidationError(
                f"Tempo {bpm} BPM out of range [{TEMPO_MIN}-{TEMPO_MAX}]",
                error_code="E211",
